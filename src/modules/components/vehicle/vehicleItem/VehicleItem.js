  ...
  return (
    <div className={styles.container}>
      <div className={styles.image__div} onClick={handleViewVehicle}>
        <MDBCardImage className="img-fluid" src={`http://localhost:8080/vehicles/images/download/${vehicleImages[0].filename}`} waves />
      </div>
      <MDBCardTitle>{brand} {model}</MDBCardTitle>
      <span className={styles.price__span}>{currency.format(price)}</span>
      <MDBCardFooter>
        <div className={styles.footer__div}>
          <div><MDBIcon icon="gas-pump" /> {fuelType}</div>
          <div><MDBIcon icon="car" /> {transmissionType}</div>
        </div>
      </MDBCardFooter>
    </div>
  )
