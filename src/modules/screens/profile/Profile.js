...
import { Profile as ProfileDetails, UserDocument, UserDocumentItem } from './../../components';
...
const Profile = props => {
  ...
  const renderUserProfile = () => {
    const imageList = state.user.documents.map(
      img => <UserDocumentItem key={img.id} media={{ image: img, styles: styles.img }} />
    )
  ...
  return state.loading ? renderLoading() : state.error ? renderError() : renderUserProfile();
}
